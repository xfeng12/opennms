...
public class ShowImport extends OsgiCommandSupport {
    ...
    protected Object doExecute() throws Exception {
        final RequisitionProvider provider = registry.getProviderByType(type);
        if (provider == null) {
            throw new IllegalArgumentException(String.format("No provider found for type '%s'. Avaiable types include: %s",
                    type, registry.getTypes()));
        }
        final Map<String, String> paramMap = parse(parameters);
        final Requisition requisition = provider.getRequisition(paramMap);
