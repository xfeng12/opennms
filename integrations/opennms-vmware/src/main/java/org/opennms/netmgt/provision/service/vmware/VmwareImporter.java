...
public class VmwareImporter {
    ...
    private boolean checkForAttribute(ManagedEntity managedEntity) throws RemoteException {
        ...
        if (!request.getCustomAttributes().isEmpty()) {
            ...
            boolean ok = true;
            for (String keyName : request.getCustomAttributes().keySet()) {
                logger.debug("Looking up for custom attribute {} with value {}", keyName, request.getCustomAttributes().get(keyName));
                ...
                if (attribValue == null) {
                    ...
                    String keyValue = request.getCustomAttributes().get(keyName);
